...
;(function($) {
	...
	function updateMenuItems($row) {
		var menuItemFields = $("#judyno_manager .fld.menuitem select"),
			boxes = [],
			regex = /\[([0-9]+)\]/;
...
<div class="wrap" id="manager_wrap">
	<div class="manager_head clearfix">
		<span class="minimize_all ju_uikit_button '.($itemMinimized == 'yes'  ? 'minimized' : '').'">'.($itemMinimized == 'yes'  ? 'Maximize All' : 'Minimize All').'</span>
	...
	<span class="resize-dynoitem ju_uikit_button " title="Minimize Box">'.(($itemMinimized == 'yes' && $type =='default') ? 'Maximize Box' : 'Minimize Box').'</span>
